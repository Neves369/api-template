...
const verifyToken = async (token: string, type: string): Promise<Token> => {
  const tokenRepository = AppDataSource.getRepository(Token);
  console.log("teste: ", jwt.verify(token, config.jwt.secret))
  const payload = jwt.verify(token, config.jwt.secret);
  const tokenDoc = await tokenRepository.findOne({where: { token: token, type: type, user: payload.sub, blacklisted: false }});
  if (!tokenDoc) {
